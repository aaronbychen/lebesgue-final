...
\fancyhead[C]{Math 4102 - Final Problem List}
\fancyhead[R]{ID: 518915}
\fancyfoot[C]{\thepage}

% Title info
\title{Math 4102 - Final Problem List}
...
By Egoroff's Theorem, there is a measurable subset $C \subset C_0$ such that $m(C_0 \setminus C) < \varepsilon/4$, and $f_n \to f$ uniformly on $C$.
...
\vspace{2em}
\begin{solution}
\textbf{Definition.} A real \emph{normed linear space} is a vector space $V$ such that for each $\bar{u} \in V$, there is a unique real number $\|\bar{u}\|$ satisfying:
\begin{enumerate}
...
It is easy to verify that $\bar{u}' \cdot \bar{u}' = \bar{v}' \cdot \bar{v}' = 1$, so that by the lemma 32.12,
\[
...
\]
By the Cauchy inequality (Theorem 33.7),
\[
...
\]
Taking square roots of both nonnegative sides gives
\[
\|f+g\|_2 \le \|f\|_2 + \|g\|_2
\]
\qed
...
\vspace{2em}
\begin{solution}
Let \(f\in L^2[-\pi,\pi]\), and define the Fourier coefficients
\[
...
\[
    \lim_{k\to\infty}\int_{-\pi}^{\pi} f(x)\cos kx\,dx
    =
    \lim_{k\to\infty}\int_{-\pi}^{\pi} f(x)\sin kx\,dx
    =
    0.
\]
Equivalently,
...
\textbf{Proof.}
By Bessel's inequality, the Fourier coefficients of \(f\in L^2[-\pi,\pi]\)
satisfy
\[
    \frac{a_0^2}{2}
    +
...
\[
    \sum_{k=1}^{\infty}(a_k^2+b_k^2)
\]
is a convergent series of nonnegative terms. Therefore its terms must go to
zero:
\[
    a_k^2+b_k^2\to 0.
\]
Since \(a_k^2\geq 0\) and \(b_k^2\geq 0\), it follows that
\[
    a_k^2\to 0
    \qquad\text{and}\qquad
    b_k^2\to 0.
